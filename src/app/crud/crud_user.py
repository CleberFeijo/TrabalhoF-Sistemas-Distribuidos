...
from core.database.connections import users_collection, roles_collection
import os
from core.security import verify_password, get_password_hash
from core.permission import UserPermission


class CRUDUser(UserPermission):
    def __init__(self, user: UserCreateSchema = None):
        if not user:
            return
    ...
    def update_user(self, user_id: ObjectId, user: UserUpdateModel):
        ...
        if user.roles:
            valid_roles = [role["role"] for role in roles_collection.find()]
            roles = list(set(user.roles))  # Remove duplicates
            ...
            if invalid_roles:
                ...
                update_fields["roles"] = user.roles
